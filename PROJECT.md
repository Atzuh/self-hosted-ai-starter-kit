...
| Specialisten sequentieel (Vererving → Lasten → Burgerlijke); geen expliciete "wat heeft elke agent gevonden"-reporting | Parallelle fan-out: Deterministic Checks (beide) en Aggregate Markdowns (analyse) connecten met alle 3 `Build * Prompt` nodes tegelijk. `Specialist Merge` (v3, combineByPosition, 3 inputs) wacht op alle 3. `Collect Specialists` bouwt een `{ specialists: {vererving, lasten, burgerlijke}, summary: {...} }` payload. **Let op**: n8n's executionOrder v1 is single-threaded, dus geen wall-clock speedup — winst is architectureel + reporting. |
